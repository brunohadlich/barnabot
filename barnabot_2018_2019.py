...
from communication import Communication, send_message, send_photo
...
single_keywords_speaches = [
    [[u"vida"], [u"Sou um matuto sofrido", u"A vida (do termo latino vita) é um conceito muito amplo e admite diversas definições. Pode-se referirː ao processo em curso do qual os seres vivos são uma parte; ao espaço de tempo entre a concepção e a morte de um organismo; à condição de uma entidade que nasceu e ainda não morreu; e àquilo que faz com que um ser esteja vivo. Metafisicamente, a vida é um processo contínuo de relacionamentos."]],
    [[u'foda', u'dific', u'difíc', u'sofri', u'sofre', u'sofrê'], [u"Quando eu era um matutinho tive q trabaia pra compra meu primeiro computador.", u"O sofrimento e a dificuldade são etapas intrinsicas a vida e que são necessárias para nos fazer crescer como individuo.."]],
    [[u'$', u'dólar', u'dolar', u'dollar'], [u'Aaahh U$1,00 é tudo que eu queria.', u'O dólar americano é a moeda utilizada pelos país Estados Unidos.']],
    [[u'balde'], [u'Me chamaram?', u'O balde é um objeto geralmente composto de plastico com o intuito de poder armazenar liquidos mas que pode vir a ser utilizado para outros propósitos.']],
    [[u'merda'], [u'Alguém precisando de um balde?', u'Vamos manter a classe por favor, palavras de baixo calão não ajudam em nada.']],
    [[u'chulé', u'chule'], [u'Não fui eu', u'Chulé não é bom, favor utilizar pé de atleta.']],
    [[u'caro'], [u'Meu salário não permite, sou destitute', u'Você sabia? pessoas que são muito pobres nos EUA são chamadas de destitute']],
    [[u'salário', u'salario'], [u'Qual teu salário? Sou destitute', u'O salário é algo pessoal e que deve ser compartilhado apenas por desejo particular e não pelo interesse alheio.']],
    [[u'teclado'], [u'Teclado no bash do android?', u'Você quis dizer teclado mecânico?']],
    [[u'bash', u'shell'], [u'O bash(born again shell) é um tipo de shell.']],
    [[u'quebra', u'cadeira', u'mesa'], [u'Precisam de um manutenir?', u'Entrem em contato com uma loja especializada no reparo de mesas e cadeiras.']],
    [[u'coca', u'refri'], [u'Nossa essa me dói o figado', u'Coca é um refrigerante com alto teor de açucar e prejudicial a saúde.']],
    [[u'função', u'cargo'], [u'Sou Software Engineer', u'Sou ass monitor pleno 2', u'Sou Higienizador Senior 1', u'Sou Bombona carries junior 3', u'Sou Ethnic Terminator senior 3', u'Sou Linux Software Engineer']],
    [[u'banco'], [u'Bank of America', u'Bank of America é um cliente em potencial para a ZPE.']],
    [[u'nestor'], [u'Hi Nestor', u'Nestor é um ótimo colega de trabalho, sempre disposto a ajudar o restante do grupo.']],
    [[u'lesse', u'leu'], [u'Praticamente', u'A leitura muda vidas.']],
    [[u'alemão', u'alemao'], [u'Verschlange', u'Auf deine mutter', u'Schultz', u'No passado fiz muitas piadas com o povo alemão mas hoje sou outra pessoa.']],
    [[u'jogo', u'jogar'], [u'Jogar um Medal of Honor?', u'O que você(s) gostaria(m) de jogar?']],
    [[u'pagamento', u'cascalho'], [u'É dia 2?', u'O salário deve cair na conta até o quinto dia util do mês.']],
    [[u'android'], [u'Android usa Linux?', u'Android é um sistema operacional baseado em linux com foco em dispositivos móveis.']],
    [[u'selfie'], [u'Eu gosto de pau de selfie', u'Selfies se tornaram muito comuns com o advento dos smartphones.']],
    [[u'estados unidos', u'tio sam'], [u'Prefiro Angola', u'Já estive nos Estados Unidos']],
    [[u'força', u'forte'], [u'Eu uso pulseira de força', u'Para aumentar a força faça atividades fisicas.']],
    [[u'gord'], [u'Ser obeso é prejudicial a saúde.', u'Gordo é um termo prejorativo use Obeso']],
    [[u'tela'], [u'Screen security?', u' Espiar a tela dos outros não é nada legal']],
    [[u'tamanho'], [u'Acho que é GG', u'Depende da sua idade e peso.']],
    [[u'pobre', u'fudido'], [u'Também sou destitute', u'Já fui um dia']],
    [[u'mst'], [u'Sou presidente do MST, movimento sem trabalho', u'MST significa movimento sem terra']],
    [[u'lula', u'pt'], [u'Lula é meu senhor e pinga não me faltará', u'Lula é um ex-presidente brasileiro que agora se encontra preso por corrupção.']],
    [[u'peru', u'perú'], [u'Eu queria dois perus, um pra comer e o outro pra ...', u'Peru é uma ave muito saborosa']],
    [[u'piroca'], [u'Prefiro pau de selfie', u'Olha o palavreado']],
    [[u'kernel'], [u'O kernel é escrito em Angular?', u'Kernel é uma palavra utilizada em computação para definir o núcleo de um sistema operacional']],
    [[u'esquecido', u'esqueci'], [u'Pô David toto mundo ja tinha equesido dizo', u'Esquecimento pode significar amnésia ou uma doença mais perigosa que deve ser verificada por um médico.']],
    ...
    [[u'linguagem', u'linguagens', u'java', u'javascript', u'delphi', u'python', u'node', u'sql'], [u'Gosto dessa linguagem', u'Eu programo em AngularJS']],
...
def send_photo_command(message, text, chat_id, com):
    try:
        if message and 'photo' in message and 'caption' in message:
            caption = message['caption']
            parts_command = caption.split(';')
            if len(parts_command) == 3:
                destination_chat_id = parts_command[1].strip()
                msg = parts_command[2].strip()
                photo = message['photo']
                file_id = photo[len(photo) - 1]['file_id']
                send_photo(msg, destination_chat_id, file_id, com)
                return True
    except:#if no send_msg is found on text an exception will occur, in this case just return False meaning the command was not executed
        None
    return False
...
def process_msg(message, text, chat_id, group_title, com):
    if (send_message_command(message, text, chat_id, com) or
        send_photo_command(message, text, chat_id, com)):
